...
public class Park_Play {
	...
	private void delete() {
		...
		if(controller.size() > 0) {
			...
			//1. 차량 번호 입력
			park.setCarNumber(validateDeleteNumber());
			
			//하지만 이 park 객체는
			//우리가 아까 입력한 객체와 일치하는 정보를 가지고 있지 않다.
			//단지 이 park의 plateNumber가 우리가 가지고 있는 리스트의 객체 중
			//하나와 일치하는 plateNumber를 가졌을 뿐이다.
			//그래서 inTime은 park에서는 0이다.
			...
			//inTime을 넣어주자
			park = controller.get(park);
			
			//이제 park 안에 inTime이 들어가있으므로
			//출차시간을 입력받아서 유효한 시간인지 확인한다.
			int outTime = validateOutTime(park);
			
			//이제 올바른 출차시간이 있으므로
			//요금 계산이 가능해진다.
			calculateRate(outTime, park);
			
			//요금계산 후에 controller 안의 list에서
			//해당 객체를 삭제해준다.
			controller.remove(park);
		}else {
	...
	private void calculateRate(int outTime, Park p) {
		//시간의 차이를 계산
		...
		System.out.println(p.getCarNumber()+" 번 차량의 주차 요금은 "+rate+"원입니다.");
		
	}
	
	private int validateOutTime(Park p) {
	...
	private void insert() {
		...
		if(controller.size() < SIZE) {
			...
			p.setCarNumber(validateInsertNumber());
